...
scle = ds["domain_yearly_mean_climatology"].attrib["scale_factor"];
oset = ds["domain_yearly_mean_climatology"].attrib["add_offset"];
...
for ii = 1 : 18
    ...
    ds = Dataset(fnca); prcp[:,:,ii] = ds["domain_yearly_mean_climatology"].var[:];
    close(ds);
end
...
plot([pi*24^2,pi*24^2],[0.27,0.32])
plot([pi*48^2,pi*48^2],[0.27,0.32])
xlabel(L"Size of Landmass / km$^2$")
ylabel(L"Mean Precipitation Rate / mm hr$^{-1}$")
xlim(10,10^6); ylim(0.27,0.32);
grid("on"); gcf()
savefig("$(plotsdir("islandmeanprcp-0.5smooth.png"))",bbox_inches="tight")
